...
const suppliersData = [
  { name: 'Golden Crust Bakery', location: 'Chennai', category: 'Restaurants and Cafes' },
  { name: 'Saravana Bhavan Outlet', location: 'Chennai', category: 'Restaurants and Cafes' },
  { name: 'Spice Route Kitchen', location: 'Coimbatore', category: 'Caterers & Hotels' },
  { name: 'Madurai Meals Hub', location: 'Madurai', category: 'Schools & Institutions' },
  { name: 'Grand Celebration Hotel', location: 'Bangalore', category: 'Weddings and Parties' },
  { name: 'Corporate Catering Solutions', location: 'Hyderabad', category: 'Corporate Offices & Canteens' },
  { name: 'Royal Events Catering', location: 'Chennai', category: 'Caterers & Hotels' },
]

async function setupSuppliersAndGenerateCSV() {
  console.log('📋 Setting up suppliers in database...')
  ...
  try {
    ...
    const existingNames = new Set(existingSuppliers.map(s => s.supplier_name))
    const suppliersToInsert = suppliersData.filter(s => !existingNames.has(s.name))

    let allSuppliers = existingSuppliers

    if (suppliersToInsert.length > 0) {
      console.log(`⚡ Inserting ${suppliersToInsert.length} new suppliers...`)
      const { data: newSuppliers, error: insertError } = await supabase
        .from('suppliers')
        .insert(suppliersToInsert.map(s => ({
      ...
      allSuppliers = [...existingSuppliers, ...newSuppliers]
    } else {
      console.log('✅ All suppliers already exist')
    }

    // Create mapping of supplier names to UUIDs
    const supplierMap = {}
    allSuppliers.forEach(s => {
      supplierMap[s.supplier_name] = s.supplier_id
    })

    console.log('\n📊 Supplier UUID Mappings:')
    Object.entries(supplierMap).forEach(([name, uuid]) => {
      console.log(`  ${name}: ${uuid}`)
    })

    // Now generate CSV with real UUIDs
    console.log('\n🔄 Generating CSV with real supplier UUIDs...')
    generateCSVWithUUIDs(supplierMap)

    console.log('✅ Setup and CSV generation complete!')
  } catch (error) {
...
function generateCSVWithUUIDs(supplierMap) {
  const outputPath = path.resolve(__dirname, '../data/food_logs_sample_365.csv')

  const suppliers = Object.entries(supplierMap).map(([name, uuid]) => {
    const mapping = {
      'Golden Crust Bakery': { uuid, businessType: 'Restaurants and Cafes' },
      'Saravana Bhavan Outlet': { uuid, businessType: 'Restaurants and Cafes' },
      'Spice Route Kitchen': { uuid, businessType: 'Caterers & Hotels' },
      'Madurai Meals Hub': { uuid, businessType: 'Schools & Institutions' },
      'Grand Celebration Hotel': { uuid, businessType: 'Weddings and Parties' },
      'Corporate Catering Solutions': { uuid, businessType: 'Corporate Offices & Canteens' },
      'Royal Events Catering': { uuid, businessType: 'Caterers & Hotels' },
    }
    return { supplier_id: uuid, supplier_name: name, ...mapping[name] }
  })
  ...
  function getSupplierSpecificFactors(supplierId) {
    const supplierByUUID = suppliers.reduce((acc, s) => {
      acc[s.uuid] = s
      return acc
    }, {})

    const supplier = supplierByUUID[supplierId]
    if (!supplier) return { basePrepare: 100, weekendMultiplier: 1.1, festivalMultiplier: 1.05 }

    const factors = {
      'Restaurants and Cafes': { basePrepare: 100, weekendMultiplier: 1.2, festivalMultiplier: 1.1 },
      'Caterers & Hotels': { basePrepare: 180, weekendMultiplier: 1.6, festivalMultiplier: 1.6 },
      'Schools & Institutions': { basePrepare: 95, weekendMultiplier: 0.85, festivalMultiplier: 0.9 },
      'Weddings and Parties': { basePrepare: 280, weekendMultiplier: 2.2, festivalMultiplier: 2.8 },
      'Corporate Offices & Canteens': { basePrepare: 200, weekendMultiplier: 0.6, festivalMultiplier: 1.8 },
    }

    return factors[supplier.businessType] || { basePrepare: 100, weekendMultiplier: 1.1, festivalMultiplier: 1.05 }
  }
  ...
  function buildRows() {
    ...
    for (let i = 0; i < 365; i += 1) {
      ...
      const supplierTrend = (i / 365) * (supplier.supplier_id === suppliers[0].supplier_id ? -0.04 : 0.02)
